...
const Wallet: React.FC<Props> = ({ balance }) => {
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      } else if (data.success && data.payUrl) {
        window.open(data.payUrl, '_blank');
      } else {
  ...
  return (
    <div>
      ...
      <input type="number" placeholder="Или введите сумму" value={amount} onChange={(e) => setAmount(e.target.value)} style={{ width: '100%', padding: '14px', marginBottom: '12px', background: '#1a2332', border: '1px solid #2a3a4a', borderRadius: '12px', color: '#fff', fontSize: '15px', boxSizing: 'border-box', textAlign: 'center' }} />
